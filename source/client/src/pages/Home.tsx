...
export default function Home() {
  ...
  return (
    <main className="site-shell">
      ...
      <section id="resumes" className="paper-section resumes-section"><div className="container"><SectionIntro kicker={t.resumesKicker} title={isArabic ? "ثلاث زوايا لملف واحد." : "Three lenses for one professional file."} body={isArabic ? "السير الذاتية التي لم تصل بعد تظهر كمواضع جاهزة للرفع، أما الملف المتاح فعلياً فيُفتح مباشرة من هنا." : "Resumes not yet supplied remain as upload slots; the file that is available opens directly here."} number="10" /><div className="resume-grid">{[{ titleAr: "Master Professional CV", titleEn: "Master Professional CV", audienceAr: "الصورة الأشمل", audienceEn: "The complete picture", ready: false }, { titleAr: "Driver & Fleet Management CV", titleEn: "Driver & Fleet Management CV", audienceAr: "لأدوار القيادة والإشراف", audienceEn: "Driving and supervision roles", ready: false }, { titleAr: "Digital / AI / Marketing CV", titleEn: "Digital / AI / Marketing CV", audienceAr: "للمشروعات والعمل الرقمي", audienceEn: "Digital and project work", ready: false }].map((resume, index) => <article className="resume-card reveal reveal-delay-1" key={resume.titleAr}><div className="resume-cover"><span>CV / 0{index + 1}</span><div className="resume-cover-mark"><img src={assets.mark} alt="" /><span>EH</span></div><strong>{isArabic ? resume.titleAr : resume.titleEn}</strong></div><div className="resume-card-body"><p>{isArabic ? resume.audienceAr : resume.audienceEn}</p><span className="text-link muted-link">{t.pending} <FileText size={15} /></span></div></article>)}</div><div className="available-file reveal"><div><FileCheck2 size={19} /><div><p className="eyebrow">{isArabic ? "ملف متاح حالياً" : "Available now"}</p><h3>{isArabic ? "Courses & Award PDF" : "Courses & Award PDF"}</h3></div></div><a className="text-link" href={assets.coursesAwards} {...externalProps}>{t.openPdf} <ArrowUpRight size={15} /></a></div></div></section>
